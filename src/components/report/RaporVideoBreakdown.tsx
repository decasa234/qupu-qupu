  ...
  return (
    <div className="mt-3 space-y-2">
      {withVideos.map((s) => (
        ...
        >
          <summary className="flex cursor-pointer items-center justify-between gap-2 text-[0.625rem] font-extrabold uppercase tracking-[0.1em]">
            <span style={{ color: s.colorHex }}>{s.name}</span>
            <span className="text-qupu-muted transition-transform">▶</span>
          </summary>
          <ul className="mt-2 space-y-1 text-[0.6875rem] text-qupu-brand-blue">
            {s.videos.map((v) => (
              <li key={v.videoId}>
