...
/**
 * WMI-19F2A-Q8 — a clean analog clock face reading 06:45.
 * Minute hand points to 9 (45 min → 270°); hour hand is three-quarters of the
 * way from 6 to 7 (6 × 30 + 45/60 × 30 = 202.5°), i.e. just before 7.
 */
export function ClockFaceG2({ emphasize = 'none' }: ClockFaceProps) {
  const minuteAngle = 270 // points to the 9
  const hourAngle = 202.5 // between 6 and 7, closer to 7
  ...
  return (
    <svg viewBox="0 0 150 150" width={150} height={150} aria-hidden="true" style={{ overflow: 'visible' }}>
      ...
      {/* Numbers 12 / 3 / 6 / 9 */}
      {([12, 3, 6, 9] as const).map((num) => {
        const pos = clockHandPoint((num % 12) * 30, 46)
        return (
          <text key={num} x={pos.x} y={pos.y} textAnchor="middle" dominantBaseline="central" fontSize={11} fontWeight="bold" fill={PURPLE}>
            {num}
          </text>
        )
      })}

      {/* Minute hand (long) — points to the 9 */}
      <line
        x1={75}
        y1={75}
        x2={minuteTip.x}
      ...
      {/* Hour hand (short) — between 6 and 7, just before 7 */}
      <line
        x1={75}
        y1={75}
        x2={hourTip.x}
...
export default function ClockReadG2Illustration() {
  return (
    <div
      className="my-4 overflow-hidden rounded-lg border-2 border-qupu-cream-dark bg-white p-2"
      role="img"
      aria-label="A clock showing 06:45"
    >
