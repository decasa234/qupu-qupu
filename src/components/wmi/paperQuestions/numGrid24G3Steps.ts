...
import {
  ...
  SOLVED,
  type Marker,
} from './NumGrid24G3Illustration'

// WMI-24F3A-Q25 — solve the number sum-grid one marked cell at a time. Each
...
export type NumGridPhase = 'intro' | 'bullet' | 'diamond' | 'star' | 'plug' | 'result'

export interface NumGridStep {
  phase: NumGridPhase
...
export interface NumGridStoryboard {
  ...
  const steps: NumGridStep[] = [
    {
      ...
      revealed: [],
      caption: t(
        'Each grey circle = the three squares above it. Solve one marked column at a time!',
        'Tiap lingkaran abu-abu = tiga kotak di atasnya. Selesaikan satu kolom bertanda sekaligus!',
      ),
      hold: 2600,
      result: false,
    },
...
export { GLYPH as NUM_GLYPH, NAME as NUM_NAME }
