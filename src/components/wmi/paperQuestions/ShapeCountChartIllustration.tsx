...
export const SHAPE_ROWS: ShapeRow[] = [
...
const SCATTER_DX = 30
const SCATTER_DY = 34
const SCATTER_W = SHAPE_VIEW_W * 0.46
...
export function scatterItems(): ScatterItem[] {
...
export function ShapeCountChart({ activeRow = null, showAll = false }: ShapeChartProps) {
  return (
    <svg
      viewBox={`0 0 ${SHAPE_VIEW_W} ${SHAPE_VIEW_H}`}
      width="100%"
      style={{ maxWidth: 480, display: 'block', margin: '0 auto' }}
      aria-hidden="true"
    >
      {/* Divider between scatter and chart */}
      <line x1={SCATTER_W + 12} y1={10} x2={SCATTER_W + 12} y2={SHAPE_VIEW_H - 10} stroke="#E2E8F0" strokeWidth={2} />

      {/* Scatter group */}
      {scatterItems().map((it, i) => (
        <ScatterShape key={i} item={it} dim={activeRow != null && !showAll && SHAPE_ROWS[activeRow].kind !== it.kind} />
      ))}
      ...
      <line
        ...
        y1={CHART_Y0 - 8}
        x2={CHART_X}
        y2={CHART_Y0 + SHAPE_ROWS.length * ROW_H - 8}
        stroke="#94A3B8"
        strokeWidth={2}
      />
      ...
      {/* Axis tick label */}
      <text x={CHART_X + AXIS_MAX * UNIT} y={CHART_Y0 + SHAPE_ROWS.length * ROW_H + 4} textAnchor="middle" fontSize={11} fill="#94A3B8">
        {AXIS_MAX}
      </text>
    </svg>
  )
