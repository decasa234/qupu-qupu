// Storyboard for WMI-23F2A-Q25 (2023 Grade 2 Final).
//
...
// (This maze is geometrically identical to the 2023 G1 Q25 maze — same walls,
//  entrance/exit, route and answer — so the method taught is the same.)
//
...
// The answer is derived as OPTIMAL_ROUTE.length, never hardcoded.
//
// Pure (lang) => storyboard. Deterministic: walks the fixed route in order, no
// Math.random / Date. SSR-safe.

import type { Lang } from '../../concepts/explainers/makeTenSteps'
import { OPTIMAL_ROUTE, type Cell } from './RobotMaze23G2Illustration'
...
export interface RobotMazeStep {
  ...
  /** How many cells of the route to reveal (1..route.length) — feeds Maze23G2 `step`. */
  step: number
...
export interface RobotMazeStoryboard {
  /** The final answer = least squares passed (20), derived from route length. */
  answer: number
  steps: RobotMazeStep[]
  finalIndex: number
}
...
export function buildRobotMaze23G2Steps(lang: Lang): RobotMazeStoryboard {
  const t = (en: string, id: string) => (lang === 'id' ? id : en)

  const route = OPTIMAL_ROUTE
  const answer = route.length // 20 — derived from the route, not hardcoded
