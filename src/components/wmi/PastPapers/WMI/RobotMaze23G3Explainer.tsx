...
// same scene coming alive. The traced route + robot are drawn by the RobotMaze23G3
// primitive (route + robot in fill-qupu-brand-orange / fill-qupu-brand-blue),
// driven here square-by-square via its `path` + `step` props.
const ORANGE = '#f0853a' // fill-qupu-brand-orange (route / count accent / right turn)
const BLUE = '#30598A' // fill-qupu-brand-blue (intro / straight accent)
const GREEN = '#10B981' // result accent (the exit)
const PURPLE = '#7C5CBF' // revisit accent (a square forced to be used again)

export default function RobotMaze23G3Explainer(props: ExplainerProps) {
  const lang = props.lang ?? 'en'
  const story = useMemo(() => buildRobotMaze23G3Steps(lang), [lang])
  ...
  const ariaLabel = t(
    `Explainer: walk the shortest legal route through the 7×5 square maze one square at a time. The robot may only go straight or turn right, so it is forced back across ${story.revisitCount} squares it already used — and every square counts, even the repeats. Counting all ${SOLUTION_PATH.length} squares, the least number passed through is ${story.answer}.`,
    `Penjelasan: telusuri rute sah terpendek di labirin persegi 7×5 satu kotak demi satu kotak. Robot hanya boleh lurus atau belok kanan, jadi terpaksa melewati lagi ${story.revisitCount} kotak yang sudah dipakai — dan setiap kotak dihitung, termasuk yang berulang. Menghitung semua ${SOLUTION_PATH.length} kotak, paling sedikit yang dilewati adalah ${story.answer}.`,
  )
  ...
  return (
    <div className="mx-auto w-full max-w-[320px]" role="img" aria-label={ariaLabel}>
      <div className="flex flex-col items-center gap-3">
        ...
        <RobotMaze23G3 path={SOLUTION_PATH} step={beat.step} />
