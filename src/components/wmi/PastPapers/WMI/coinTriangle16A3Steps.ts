// SEAMO-16-A-Q3 — storyboard for the coin-triangle animation.
//
// Vanessa uses coins to form a triangle. Each of the 3 corners has a coin;
// there are 6 coins on each side. Total = 3 × 6 − 3 = 15. Answer: D.
//
// Teaching walk — one idea per beat:
//   0. intro   — show the coin triangle; state the problem.
//   1. sides   — highlight one side and show 6 coins per side.
//   2. naive   — naive count: 3 × 6 = 18; but corners counted twice.
//   3. corners — spotlight the 3 corner coins that are double-counted.
//   4. fix     — subtract 3 corners: 18 − 3 = 15.
//   5. result  — confirm answer 15, choice D.
//
...
export function buildCoinTriangle16A3Steps(lang: Lang): TriangleStoryboard {
  ...
  const steps: TriangleBeat[] = [
    // Beat 0 — intro
    {
      ...
      result: false,
      caption: t(
        'Vanessa arranges coins in a triangle. There are 6 coins on each side, and each corner coin is shared by two sides.',
        'Vanessa menyusun koin membentuk segitiga. Ada 6 koin di setiap sisi, dan koin sudut digunakan bersama oleh dua sisi.',
      ),
    },
    ...
    // Beat 5 — result
    {
      ...
      cornerColor: GREEN,
      edgeColor: GREEN,
      equation: t('15 coins → Answer D', '15 koin → Jawaban D'),
      hold: 0,
      result: true,
      caption: t(
        'Vanessa uses 15 coins in total. Answer D.',
        'Vanessa menggunakan 15 koin. Jawaban D.',
      ),
    },
  ]
