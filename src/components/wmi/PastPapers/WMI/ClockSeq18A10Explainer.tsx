// SEAMO-18-A-Q10 — animated explainer for the clock sequence puzzle.
//
// "What time should come next in the sequence below?"
// Sequence: 2:30 → 3:30 → 4:30 → ?   (each +60 min / +1 hour)
// Answer: C = 5:30 PM.
//
// Reuses ClockSeq18A10 from the illustration for the 2×2 grid, and
// AnalogClock20 for the optional focused single-clock highlight.
//
...
import { ClockSeq18A10 } from './ClockSeq18A10Illustration'
import { buildClockSeq18A10Steps } from './clockSeq18A10Steps'
...
export default function ClockSeq18A10Explainer(props: ExplainerProps) {
  const lang = props.lang ?? 'en'
  const T = (en: string, id: string) => (lang === 'id' ? id : en)

  const story = useMemo(() => buildClockSeq18A10Steps(lang), [lang])
  ...
  const ariaLabel = T(
    'Clock sequence: 2:30, 3:30, 4:30 — each one hour later. The next time is 5:30 PM. Answer C.',
    'Urutan jam: 2:30, 3:30, 4:30 — masing-masing 1 jam kemudian. Waktu berikutnya adalah pukul 17:30. Jawaban C.',
  )
      ...
      >
        ...
        <AnimatePresence initial={false}>
          {beat.result && (
            ...
            >
              {T('5:30 PM ✓  (Answer C)', '17:30 ✓  (Jawaban C)')}
            </motion.div>
          )}
        </AnimatePresence>
