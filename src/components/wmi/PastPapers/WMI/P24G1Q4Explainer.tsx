...
import { AnimalRow } from './P24G1Q4Illustration'
import { buildP24G1Q4Steps } from './p24G1Q4Steps'
...
export default function P24G1Q4Explainer(props: ExplainerProps) {
  const lang = props.lang ?? 'en'
  const story = useMemo(() => buildP24G1Q4Steps(lang), [lang])
  ...
  const ariaLabel = T(
    `Strategy: count 10 from the left to the leader (${story.leaderEn}) and 7 from the right to the vice leader (${story.viceEn}), then pick the option holding both — answer ${story.answerLetter}.`,
    `Strategi: hitung 10 dari kiri ke pemimpin dan 7 dari kanan ke wakil, lalu pilih opsi yang memuat keduanya — jawaban ${story.answerLetter}.`,
  )
