// HKIMO-24-P1H-Q19 — animated explainer for the ●●○○ bead chain pattern.
//
// Animation flow:
...
//
// Re-uses BeadCircle, BeadType, CHAIN, BLANK_INDEX, beadCx, beadR, CY
// from BeadChainHK24P1Q19Illustration.
...
import { buildBeadChainHK24P1Q19Steps } from './beadChainHK24P1Q19Steps'
...
const SVG_W = beadCx(CHAIN.length - 1) + beadR(CHAIN[CHAIN.length - 1]) + 20
const SVG_H = 76

// ─── Component ───────────────────────────────────────────────────────────────

export default function BeadChainHK24P1Q19Explainer(props: ExplainerProps) {
  const lang  = props.lang ?? 'en'
  const story = useMemo(() => buildBeadChainHK24P1Q19Steps(lang), [lang])
  ...
  const ariaLabel =
    lang === 'id'
      ? 'Penjelasan: pola ●●○○ berulang — posisi ke-10 adalah ke-2 dalam unit, jadi jawabannya ● (hitam besar).'
      : 'Explainer: pattern ●●○○ repeats — position 10 is 2nd in the unit, so the answer is ● (big black bead).'
      ...
      <div className="flex flex-col items-center gap-3">
        ...
        >
          ...
          {CHAIN.map((type, i) => {
            ...
            const renderType = isFilled ? ANSWER : (type as 'B' | 'W')
