...
import {
  buildNumGrid24G3Steps,
  NUM_GLYPH,
  NUM_NAME,
} from './numGrid24G3Steps'
import {
  CELL,
  CIRCLE_R,
  COL_SUMS,
...
// WMI-24F3A-Q25 post-answer explainer. Mirrors the static number sum-grid and
...
export default function NumGrid24G3Explainer(props: ExplainerProps) {
  const lang = props.lang ?? 'en'
  const T = (en: string, id: string) => (lang === 'id' ? id : en)

  const story = useMemo(() => buildNumGrid24G3Steps(lang), [lang])
      ...
      <div className="flex flex-col items-center gap-3">
          ...
          {(['bullet', 'diamond', 'star'] as Marker[]).map((m) => {
            ...
            return (
              <motion.span
                ...
                transition={{ duration: 0.25 }}
              >
                <span style={{ color: done ? GREEN_INK : MARK }}>{NUM_GLYPH[m]}</span>
        ...
        {beat.marker != null && (
          <div className="font-display text-xs font-bold" style={{ color: BRAND_BLUE }}>
            {T(`solving the ${NUM_NAME[beat.marker][0]}`, `mencari ${NUM_NAME[beat.marker][1]}`)}
          </div>
        )}
