...
-- Materialized per-child per-concept comprehension — migration 0033
...
-- ─────────────────────────────────────────────────────────────────────
-- WMI gamification wiring (migration 0037)
-- Garden-first daily quest trio + garden/streak achievements. The new
-- event types (KONSEP_SESSION_COMPLETED, KONSEP_QUESTION_ANSWERED,
-- KONSEP_CONCEPT_GROWN, CHAPTER_TEST_PASSED) need no DDL —
-- gamification_events.event_type is an unconstrained VARCHAR(80).
-- NOTE: must stay below the coin_reward ALTER (migration 0018 mirror).
-- ─────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────
-- Track lessons (migration 0052)
-- One-shot lesson sessions: commitLesson must reference a lesson
