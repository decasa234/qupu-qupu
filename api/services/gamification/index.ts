...
import { readStreakState, updateStreakForActivity, type StreakState } from './streakUpdater.js'
import {
...
export async function processScoreSubmission(
  ...
  // Step 1: ensure profile + resolve streak. Editing a previously-scored
  // quiz (a correction) must NOT count as a fresh day of activity, so the
  // streak only advances on a genuine first-time submission. Corrections
  // read the current streak without mutating it. Streak resolution runs
  // before the quest evaluator (which reads current_streak_days).
  await ensureProfile(client, input.childId)
  const streak = input.isCorrection
    ? await readStreakState(client, input.childId)
    : await updateStreakForActivity(client, input.childId, eventDate)
  ...
  // Step 3: first-time completion path. Decision D2: corrections do not
  // re-grant completion XP — and (this change) do not count as "doing a
  // quiz" at all. QUIZ_SCORE_SUBMITTED drives the daily "do a quiz" quests,
  // so it only fires for a genuine completion, never an edit. Order matters
  // for the reward summary: completion → high → perfect → improvement.
