...
import { buildNetProgressCyclesBreakdown } from './breakdown.js'
...
export function render(p: Params) { const net = p.up - p.down; const ans = answer(p); return { body_en: `A climber goes up ${p.up} steps, then down ${p.down} steps. This repeats ${p.cycles} times.\nFind: How many steps higher is the climber than the start?`, body_id: `Seorang anak naik ${p.up} anak tangga, lalu turun ${p.down} anak tangga. Ini diulang ${p.cycles} kali.\nCari: Berapa anak tangga lebih tinggi dari awal?`, answer_type: 'fill_in' as const, choices_en: null, choices_id: null, answer: String(ans), hint_en: `Each cycle gains ${p.up} - ${p.down} = ${net}.`, hint_id: `Setiap siklus bertambah ${p.up} - ${p.down} = ${net}.`, hint_steps_en: [`Net per cycle: ${net}.`, `${net} x ${p.cycles} = ${ans}.`], hint_steps_id: [`Bersih per siklus: ${net}.`, `${net} x ${p.cycles} = ${ans}.`], breakdown: buildNetProgressCyclesBreakdown(p) } }
