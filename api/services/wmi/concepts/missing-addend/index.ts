...
export function render(p: Params) { const sum = p.a + p.b; return { body_en: `? + ${p.b} = ${sum}\nFind: What number is the question mark?`, body_id: `? + ${p.b} = ${sum}\nCari: Bilangan apa yang menjadi tanda tanya?`, answer_type: 'fill_in' as const, choices_en: null, choices_id: null, answer: String(answer(p)), hint_en: `Undo +${p.b} by subtracting ${p.b}.`, hint_id: `Balikkan +${p.b} dengan mengurangi ${p.b}.`, hint_steps_en: [`? = ${sum} - ${p.b}`, `? = ${p.a}`], hint_steps_id: [`? = ${sum} - ${p.b}`, `? = ${p.a}`] } }
