...
export function render(p: Params) { const newDen = p.den * p.m; const ans = answer(p); return { body_en: `${p.num}/${p.den} = ?/${newDen}\nFind: What number is the question mark?`, body_id: `${p.num}/${p.den} = ?/${newDen}\nCari: Bilangan apa yang menjadi tanda tanya?`, answer_type: 'fill_in' as const, choices_en: null, choices_id: null, answer: String(ans), hint_en: `The denominator is multiplied by ${p.m}, so multiply the numerator by ${p.m}.`, hint_id: `Penyebut dikali ${p.m}, jadi pembilang juga dikali ${p.m}.`, hint_steps_en: [`${p.den} x ${p.m} = ${newDen}.`, `${p.num} x ${p.m} = ${ans}.`], hint_steps_id: [`${p.den} x ${p.m} = ${newDen}.`, `${p.num} x ${p.m} = ${ans}.`] } }
