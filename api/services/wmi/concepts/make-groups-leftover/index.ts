...
import { buildMakeGroupsLeftoverBreakdown } from './breakdown.js'
...
export function render(p: Params) { const groups = Math.floor(p.total / p.groupSize); const rem = answer(p); return { body_en: `${p.total} counters are put into groups of ${p.groupSize}.\nFind: How many counters are left over?`, body_id: `${p.total} benda dibagi ke kelompok berisi ${p.groupSize}.\nCari: Berapa benda yang tersisa?`, answer_type: 'fill_in' as const, choices_en: null, choices_id: null, answer: String(rem), hint_en: `Make ${groups} full groups of ${p.groupSize}.`, hint_id: `Buat ${groups} kelompok penuh berisi ${p.groupSize}.`, hint_steps_en: [`${groups} x ${p.groupSize} = ${groups * p.groupSize}.`, `${p.total} - ${groups * p.groupSize} = ${rem}.`], hint_steps_id: [`${groups} x ${p.groupSize} = ${groups * p.groupSize}.`, `${p.total} - ${groups * p.groupSize} = ${rem}.`], breakdown: buildMakeGroupsLeftoverBreakdown(p) } }
