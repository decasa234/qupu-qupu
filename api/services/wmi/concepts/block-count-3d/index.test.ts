...
describe('block-count-3d', () => {
  ...
  test('100 seeds: 2-4 solid monotone groups, every stack 2-3 tall, bounded by 5x5x3; answer = grand total', () => {
    for (let seed = 1; seed <= 100; seed++) {
      ...
      for (const g of p.groups) {
        ...
        expect(Math.min(...g.heights)).toBeGreaterThanOrEqual(2) // every stack >= 2 tall
        ...
        expect(cubes).toBeLessThanOrEqual(16)
      }
      const t = total(p)
      expect(t).toBeGreaterThanOrEqual(12)
      expect(t).toBeLessThanOrEqual(36)
